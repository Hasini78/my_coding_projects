...
Train* searchTrain(TrainNode* root, int trainNo) {
    ...
    if (trainNo == root->trainNo) return root->trainData;
    else if (trainNo < root->trainNo) return searchTrain(root->left, trainNo);
    else return searchTrain(root->right, trainNo);
}
...
Train* searchTrainByName(TrainNode* root, const char* name) {
    ...
    if (strcmp(root->trainData->trainName, name) == 0) return root->trainData;
...
void initializeTrains() {
    Train trains[] = {
        ...
        {1295, "Sangamitra", "Bangalore", "Danapur", 120, 0, "04:30", "20:00"},
...
void viewBookings() {
    ...
    while (temp != NULL) {
        if (temp->trainNo == trainNo) {
            printf("%-9d | %-15s | %-3d | %-6c | %-20s | %-16s | %-9s | %-9s\n",
...
int main() {
    ...
    do {
        ...
        switch (choice) {
            ...
            default: if (choice != 5) printf("Invalid choice.\n");
        }
    } while (choice != 5);
    return 0;
}
